...
def one(i):
 ...
 try:
  ...
  x=np.stack(signals);annotations=get(BASE+sid+'.txt').content;(D/(sid+'.txt')).write_bytes(annotations);epochs=[];stages=[];onsets=[]
  ...
  np.savez_compressed(dest,epochs=np.stack(epochs),stages=np.array(stages),onsets=np.array(onsets),channel_names=np.array([labels[j] for j in ix]))
  (D/(sid+'_header.bin')).write_bytes(header)
  rec.update(status='included',counts=counts,selected_channels=[labels[j] for j in ix],all_channels=labels,units=[units[j] for j in ix],start_seconds=start,record_duration=dur,fs=[float(samples[j]/dur) for j in ix],source_url=u,range=f'{nh}-{nh+records*bs-1}',range_sha256=hashlib.sha256(raw).hexdigest(),header_sha256=hashlib.sha256(header).hexdigest(),annotation_sha256=hashlib.sha256(annotations).hexdigest())
