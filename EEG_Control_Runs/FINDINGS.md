...
See README.md for the command sequence. Exact analysis decisions, formulas, scripts, input hashes, person-level outputs, and graphs are versioned or reconstructable from the documented source files. Full-length source/LEMON/CAP recordings are not duplicated here. Statistical intervals are pointwise and these are retrospective analyses with documented amendments, not a preregistered causal experiment.
